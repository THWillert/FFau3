...
; ==============================================================================
; UDF ...........: FF.au3
Global Const $_FF_AU3VERSION = "0.6.0.1b-11"
...
#cs
	V0.6.0.1b-11
	- Added: New parameter for _FFCmd (for debugging)
...
; AutoIt Version : V3.3.0.0
; Syntax ........: _FFCmd($sArg[, $iTimeOut = 30000[, $bTry = True[, $bDebug = True]]])
; Parameter(s): .: $sArg        - Any JavaScript
;                  $iTimeOut    - Optional: (Default = 30000) :
;                  $bTry        - Optional: (Default = True) : adds try/catch to $sArg
;                  $bDebug      - Optional: (Default = False) : Returns the complete error from JavaScript
; Return Value ..: Success      - String
;                  Failure      - ""
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
...
; Example .......: Yes
; ==============================================================================
Func _FFCmd($sArg, $iTimeOut = 30000, $bTry = True, $bDebug = False)
	Local Const $sFuncName = "_FFCmd"
	Local $sRet, $sError = "'_FFCmd_Err'"

	If StringLeft($sArg, 1) = "." Then $sArg = "window.content.document" & $sArg

	If $bDebug Then $sError = "e"

	If $bTry And Not StringInStr(StringLower($sArg), "try") Then $sArg = "try{" & $sArg & "}catch(e){" & $sError & ";};"
...
Func __FFIsIP(ByRef $IP, $bSubstring = False)
	...
	StringRegExp($IP, $sStart & '(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}' & $sEnd) Or_ ; IPV6
	StringRegExp($IP, $sStart & '((?:[0-9A-Fa-f]{1,4}(?::[0-9A-Fa-f]{1,4})*)?)::((?:[0-9A-Fa-f]{1,4}(?::[0-9A-Fa-f]{1,4})*)?)' & $sEnd) Or_ ; IPV6 HexCompressed
	StringRegExp($IP, $sStart & '((?:[0-9A-Fa-f]{1,4}:){6,6})(25[0-5]||2[0-4]\d||[0-1]?\d?\d)(\.(25[0-5]||2[0-4]\d||[0-1]?\d?\d)){3}' & $sEnd) Or_ ; IPV6 6Hex4Dec
